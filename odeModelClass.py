...
class ODEModel():
    ...
    def Simulate(self, treatmentScheduleList, **kwargs):
        ...
        if self.resultsDf is None or treatmentScheduleList[0][0] == 0:
            currStateVec = self.initialStateList + [0, 0] # 2 drugs ------------------
            self.resultsDf = None
        else:
            currStateVec = [self.resultsDf[var].iloc[-1] for var in self.stateVars] + [self.resultsDf['Drug1_Concentration'].iloc[-1], 
                                                                                       self.resultsDf['Drug2_Concentration'].iloc[-1]]
        resultsDFList = []
        ...
        for intervalId, interval in enumerate(treatmentScheduleList):
            ...
            currStateVec[-2] = interval[2] # drug 1 ------------------
            currStateVec[-1] = interval[3] # drug 2 ------------------
            ...
            resultsDFList.append(
                pd.DataFrame({"Time": tVec, "Drug1_Concentration": solObj.y[-2, :], "Drug2_Concentration": solObj.y[-1, :],
                              **dict(zip(self.stateVars,solObj.y))})) # 2 drugs --------------------
        ...
        if len(resultsDFList) > 0:
            ...
            resultsDf = pd.DataFrame({"Time": tVec, "Drug1_Concentration": np.zeros_like(tVec), "Drug2_Concentration": np.zeros_like(tVec),
                                     **dict(zip(self.stateVars,np.zeros_like(tVec)))}) # 2 drugs --------------------
    ...
    def Trim(self, t_eval=None, dt=1):
        ...
        for variable in [*self.stateVars, 'TumourSize', 'Drug1_Concentration', 'Drug2_Concentration']: # drug1, 2--------------
    ...
    def Plot(self, plotPops=True, n_boot=1,
        ...
        drugConcentrationVec = utils.TreatmentListToTS(
            treatmentList=utils.ExtractTreatmentFromDf(drug_data_df, timeColumn="Time",
                                                    treatmentColumn="Drug1_Concentration", 
                                                    mode="post"),
