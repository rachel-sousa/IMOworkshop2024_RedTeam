...
#         dudtVec[0] = (params['r_C']*(1-(params['beta_1']*drug))*C*abs(1-(C/(params['K']+(params['delta_1']*M_P))))) - ((params['d_1']*C*T)/(1+(params['delta_2']*M_P))) - ((params['d_2']*C*M_A)*(1+(params['beta_1']*drug)))
        dudtVec[0] = (params['r_C']*C)*(1 + params['delta_1'] * M_P) - (params['d_C']*C)*(1+(params['beta_1']*drug)) - ((params['d_1']*C*T)/(1+(params['delta_2']*M_P))) - ((params['d_2']*C*M_A)*(1+(params['beta_1']*drug)))
        dudtVec[1] = (params['alpha_A']*(C/(params['q_A']+C))*(1+(params['beta_3']*drug))) + (params['r_PA']*M_P)*(1+(params['beta_5']*drug)) - (params['r_AP']*M_A*C) - (params['d_A']*M_A)
        dudtVec[2] = (params['alpha_P']*(C/(params['q_P']+C))*(1-(params['beta_3']*drug))) - (params['r_PA']*M_P)*(1+(params['beta_5']*drug)) + (params['r_AP']*M_A*C)- (params['d_P']*M_P)
        dudtVec[3] = (params['r_T']*T*M_A*omega) - (params['d_T']*T*(1-(params["beta_4"]*drug)))
        dudtVec[4] = 0
        print((params['alpha_A']*(C/(params['q_A']+C))*(1+(params['beta_3']*drug))) , (params['alpha_P']*(C/(params['q_P']+C))*(1-(params['beta_3']*drug))))
#         print(C, M_A, M_P, T)
        return (dudtVec)
