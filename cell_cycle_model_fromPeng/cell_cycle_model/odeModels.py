...
def create_model(modelName,**kwargs):
    funList = {"GDRSModel":GDRSModel,
               ...
               "CellCycleModel1":CellCycleModel1}
...
# -----------------------------------------
class CellCycleModel1(ODEModel):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.name = "CellCycleModel1"
        self.paramDic = {**self.paramDic,
            'Emax_P': 1.9721, 'EC50_P': 0.15942, 'h_P1': 0.40941,
            'Imax_P': 1, 'IC50_P': 0.041212, 'h_P2': 0.10945,
            'Imax_A': 0.92714, 'IC50_A': 5.3318, 'h_A1': 5.1795,
            'Kmax_A': 2.8866, 'KC50_A': 0.14143, 'h_A2': 5.6103,
            'K_1': 0.050779, 'K_2': 0.17906, 'K_3': 0.39181,
            'K_4': 1.1443, 'K_5': 0.060188, 'K_6': 0,
            'K_7': 2.1526, 'K_8': 0, 'p': 0.14826,
            'q': 0.45458, 'L': 13727.7261, 
            'G10': 0.01, 'S0': 0, 'SD0': 0, 'G20': 0, 'G2D0': 0,
            'Dead0': 0}
        self.stateVars = ['G1', 'S', 'SD', 'G2', 'G2D', 'Dead']
    ...
    def ModelEqns(self, t, uVec):
        #print(uVec)
        G1, S, SD, G2, G2D, Dead, D1, D2 = uVec
        dudtVec = np.zeros_like(uVec)
        p = self.paramDic['p']
        q = self.paramDic['q']
        L = self.paramDic['L']
        ...
        # Differential equations
        dudtVec[0] = 2 * k_vals[4] * (1 - (G1 + S + SD + G2 + G2D + Dead) / L) * G2 - k_vals[0] * G1 #dG1_dt
        dudtVec[1] = k_vals[0] * (1 - p * (1 + self.E_PARP1(D1))) * G1 - k_vals[1] * S + k_vals[2] * SD #dS_dt
        dudtVec[2] = k_vals[0] * p * (1 + self.E_PARP1(D1)) * G1 - k_vals[2] * SD - k_vals[3] * (1 + self.E_ATR1(D2)) * SD #dSD_dt
        dudtVec[3] = k_vals[1] * (1 - q * (1 + self.E_ATR2(D2))) * S - k_vals[4] * (1 - (G1 + S + SD + G2 + G2D + Dead) / L) * G2 + k_vals[6] * G2D #dG2_dt
        dudtVec[4] = k_vals[1] * q * ((1 + self.E_ATR2(D2))) * S - k_vals[6] * G2D #dG2D_dt
        dudtVec[5] = k_vals[3] * SD  #dDead_dt
        dudtVec[6] = 0 #dD1_dt
        dudtVec[7] = 0 #dD2_dt
        return (dudtVec)
