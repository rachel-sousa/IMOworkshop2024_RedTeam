...
class ODEModel():
    def __init__(self, **kwargs):
        ...
        self.solverMethod = kwargs.get('method', 'BDF')  ### Rachel changed this # ODE solver used
    ...
    def Plot(self, plotPops=True, n_boot=1,
        ...
        if ax is None: fig, ax = plt.subplots(1, 1, figsize=(8,5)) ### Rachel added figsize
        # varsToPlotList = ['TumourSize']
        # if plotPops: varsToPlotList += self.stateVars
        varsToPlotList = self.stateVars ######### Rachel changed this
        ...
        ax.set_xlabel("Time (Days)" if decorateAxes else "")
        ax.set_ylabel("Cell Number" if decorateAxes else "")
        ax2.set_ylabel(r"Drug Concentration in $\mu M$" if decorateAxes else "")
        ax.set_title(kwargs.get('title', ''))
        ax.legend(loc='upper right', bbox_to_anchor=(1.25, 1.0)) ############# Rachel added this
        plt.tight_layout()
