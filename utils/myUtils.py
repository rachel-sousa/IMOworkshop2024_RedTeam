...
def ConvertTDToTSFormat(timeVec, drugIntensityVec, mode="post", passaging=True):
    treatmentScheduleList = [] # Time intervals in which we have the same amount of drug
    tStart = timeVec[0]
    currDrugIntensity = drugIntensityVec[0]
    t_previous = timeVec[0]
    for i,t in enumerate(timeVec):
        if passaging: # Break schedule at every measurement as every measurement corresponds to a new passage
            if mode=="post":
            ...
            if mode=="pre":
                ...
                treatmentScheduleList.append([tStart,t,currDrugIntensity])
                tStart = t
        elif drugIntensityVec[i]!=currDrugIntensity and not (np.all(np.isnan([drugIntensityVec[i],currDrugIntensity]))): # Check if amount of drug has changed
            # Break schedule at every change in drug concentration
            if mode=="post":
                treatmentScheduleList.append([tStart,t,currDrugIntensity])
                tStart = t
            elif mode=="pre":
                treatmentScheduleList.append([tStart,t_previous,currDrugIntensity])
                tStart = t_previous
            currDrugIntensity = drugIntensityVec[i]
        t_previous = t
...
def ExtractTreatmentFromDf(dataDf,timeColumn="Time",treatmentColumn="DrugConcentration", 
    ...
    return ConvertTDToTSFormat(timeVec, drugIntensityVec, mode=mode, passaging=passaging)
...
def TreatmentListToTS(treatmentList,tVec):
    drugConcentrationVec = np.zeros_like(tVec, dtype=float)
...
def plot_drug_bar(drug_data_df, ax, timeColumn="Time", drug_columns=["Drug1_Concentration", "Drug2_Concentration"], 
    ...
    drug_bar_height = 1 - drug_bar_position
    curr_drug_bar_position = drug_bar_position

    for i, treatmentColumn in enumerate(drug_columns):
        drugConcentrationVec = TreatmentListToTS(
            treatmentList=ExtractTreatmentFromDf(drug_data_df, timeColumn=timeColumn,
        ...
        ax2.fill_between(drug_data_df[timeColumn], curr_drug_bar_position, drugConcentrationVec,
                         step="post", color=drugBarColours[i], alpha=alphas[i], 
                         label=f"{treatmentColumn} Concentration", zorder=zorder)
        
        # Add frame by plotting the borders
        ax2.plot(drug_data_df[timeColumn], np.ones_like(drug_data_df[timeColumn])*curr_drug_bar_position, color='black', lw=2, zorder=zorder+1)
        ax2.plot(drug_data_df[timeColumn], np.ones_like(drug_data_df[timeColumn])*(curr_drug_bar_position + drug_bar_height / n_drugs), color='black', lw=2, zorder=zorder+1)
        for x_val in np.where(np.abs(np.diff(drugConcentrationVec))>0):
            ax2.vlines(x=drug_data_df[timeColumn].iloc[x_val+1], 
                       ymin=curr_drug_bar_position, ymax=curr_drug_bar_position + drug_bar_height / n_drugs,
                       color='black', lw=2, zorder=zorder+1)        
        curr_drug_bar_position += drug_bar_height / n_drugs
    # Add beginning and end bounding box
    for x_val in [drug_data_df[timeColumn].iloc[0], drug_data_df[timeColumn].iloc[-1]]:
        ax2.vlines(x=x_val, 
            ymin=drug_bar_position, ymax=1,
            color='black', lw=2, zorder=zorder+1)
